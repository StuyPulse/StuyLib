...
/**
 * Integral is a class that aids in approximating integrals.
 * The Integral object stores points on the graph of which to approximate the integral of.
 * Depending on how it is configured, it can automatically update the answer when fed new points.
 * 
...
public class Integral {
     ...
     * updates the answer on adding points, but not through recalculating the entire integral approximation.
     */
    public Integral() {
    ...
    public static class Config {
        ...
        /**
         * Sets if the answer to the approximation should be updated every time a point is added.
        ...
        /**
         * Sets if the answer to the approximation should be fully recalculated.
    ...
    public interface Formula {
        ...
        /**
         * The Riemann sum approximates the integral using rectangles.
         * Over each interval [a, b], the rectangle is height f(a) and width b - a.
         * This produces area f(a) * (b - a).
         * It is the least accurate of the provided Formulas.
        ...
        /**
         * The trapezoidal method approximates the integral using trapezoids.
         * Over each interval [a, b], the trapezoid is height b - a and has bases f(a) and f(b).
         * Therefore, the area of each trapezoid is (f(a) + f(b)) * (b - a) / 2.
         * It is more accurate than Simpson for linear data.
         * Thus, this is the recommended method if you do not plan on recalculating the integral
         * over all the points every time you update, because the best approximation between two points
         * is a line.
        ...
        /**
         * The Simpson's rule for approximating integrals approximates the shape 
         * of the graph using quadratics.  Refer to the wikipedia article for more 
         * information.
         * It is more accurate than trapezoidal for curved data.
    ...
    /**
     * Uses binary search to find the index at which the point should be inserted for the x-coordinates to be in order.  
     * Not in stable order.
     * 
     * @param point
     */
    private void binarySearchInsert(Vector2D point) {
        ...
        while(low <= high) {
            ...
            if(point.x <= mDataPoints.get(mid).x && (mid < 1 || point.x >= mDataPoints.get(mid - 1).x)) {
                ans = mid;
                break;
            } 
    ...
    /**
     * Adds the integral over the last two points to the answer - used for updating without recalculating the entire approximation.
     */
    private void calculateLast() {
        ArrayList<Vector2D> lastTwo = new ArrayList<Vector2D>();
        for(int i = Math.max(0, mDataPoints.size() - 2); i < mDataPoints.size(); ++i) {
    ...
    /**
     * Get the answer to the approximation of the integral.  If alwaysUpdate is off, it could be very inaccurate.
     * 
